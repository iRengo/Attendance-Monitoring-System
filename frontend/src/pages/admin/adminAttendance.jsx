...
export default function AdminAttendance() {
  ...
  const {
    ...
    teachersInSection,
    loadingTeachers,
  } = useTeachers(classes, selectedSection);
  const {
    studentsInSection,
    loadingStudents,
  } = useStudents(selectedSection);
  ...
  const {
    ...
    loadingSessions,
  } = useSessions(selectedSection, selectedTeacher);

  // Teacher list source
  const teachersSource = selectedSection ? teachersInSection : allTeachers;

  // Teacher filtering & paging
  ...
  const teachersPageData = filteredTeachers.slice(
    (teachersPage - 1) * TEACHERS_PAGE_SIZE,
    teachersPage * TEACHERS_PAGE_SIZE
  );

  // Student filtering & paging
  ...
  // Sessions for selected teacher
  const teacherDetailSessions = useMemo(() => {
    if (!selectedTeacher) return [];
    if (selectedSection) {
      return sectionSessions.filter(
  ...
  function exportTeacherCSV() {
    if (!selectedTeacher) return;
    const groups = groupSessionsBySubject(teacherDetailSessions, classes);
    const headers = [
      "Subject",
      "SessionID",
      "ClassID",
    ...
    const rows = [];
    Object.entries(groups).forEach(([subject, sessions]) =>
      sessions.forEach((s) => {
    ...
    link.download = `TeacherSessions_${selectedTeacher.teacherId}_${
  ...
  const teacherGroups = useMemo(
    () => groupSessionsBySubject(teacherDetailSessions, classes),
    [teacherDetailSessions, classes]
  );
  const studentGroups = useMemo(
    () => groupSessionsBySubject(studentDetailSessions, classes),
    [studentDetailSessions, classes]
  );
...
// Helper grouping (kept local to page)
function groupSessionsBySubject(sessions, classes) {
